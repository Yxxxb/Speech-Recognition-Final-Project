...
- 模型
  ...
  ![](情感识别.assets/loss.png)

  ![](情感识别.assets/accuracy.png)
...
- 创建数据
  ...
  ```python
  df_empty = pd.DataFrame(columns=["path", "label"])
  count = 0
  test_list = "test_list"
  for direct in os.listdir(test_list):
      dir_path = os.path.join(test_list, direct)
      for file in os.listdir(dir_path):
          file_path = os.path.join(dir_path, file)
          label = file[7]
          if label == '5' and direct == "female":
              label = 0
              df_empty.loc[count] = [file_path, label]
              count += 1
          if label == '2' and direct == "female":
              label = 1
              df_empty.loc[count] = [file_path, label]
              count += 1
  ....
